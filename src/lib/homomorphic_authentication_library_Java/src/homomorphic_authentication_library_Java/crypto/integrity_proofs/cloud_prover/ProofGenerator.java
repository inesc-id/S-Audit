...
class ProofGenerator {
    ...
    Collection<Proof> generateProof(Challenge c, HashMap<String, UserAuditingObject> user_data) {
        ...
        Collection<UserChallenge> u_c_list = new ArrayList<UserChallenge>(c.getUser_challenge_set());
        ...
        for (FileChallenge f_c : c.getFiles()) {
            ...
            if (f_c.getFile_blocks().isEmpty()) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    byte[][] l = DataManipulator.split_message(block_content, pairing.getZr()
                            .getLengthInBytes());
                    for (byte[] s : l) {
        ...
        for (FileChallenge f_c : c.getFiles()) {
            ...
            if (f_c.getFile_blocks().isEmpty()) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    Element beta_f_c = pairing.getG1().newElementFromBytes(sig_content);
                    beta_f_c.powZn(chal);
        ...
        for (FileChallenge f_c : c.getFiles()) {
            ...
            if (f_c.getFile_blocks().isEmpty()) {
                ...
                for (FileBlockChallenge b : f_c.getFile_blocks()) {
                    ...
                    //int granularity = b.getGranularity();
                    byte[] block_content = FileBlockHasher.hashFileBlocks(b_o.getBlock_content(b
                            .getIndex()), granularity, pairing.getZr().getLengthInBytes(), d);
                    byte[][] l = DataManipulator.split_message(block_content, pairing.getZr()
                            .getLengthInBytes());
                    for (byte[] s : l) {
                        Element h_mk = pairing.getZr().newElementFromHash(s, 0, s.length);
