 ...
 */
public class BLSOptimized extends
        BLS {

    public BLSOptimized(PairingParameters params, Element g, Element w) {
        super();
        this.w = w;
    ...
    public byte[][] sign(byte[] message, String id, CipherParameters privateKey) {
        ...
        signer = new BLSSignerOptimized(new SHA256Digest(), w);
        signer.init(true, privateKey);

        for (int i = 0; i < num_splits; i++) {
            String block_id = (id + "" + i);
            byte[] bytes_id = block_id.getBytes();
            signature[i] = this.sign_block(splitMessage[i], bytes_id, signer);
        }
    ...
    public boolean verify(byte[] signature, String message, String id, CipherParameters publicKey) {
        ...
        BLSSignerOptimized signer = new BLSSignerOptimized(new SHA256Digest(), w);
    ...
    public boolean verify(byte[][] signature,
                          ...
                          Element publicKey,
                          Pairing pairing) {
        BLSSignerOptimized signer = new BLSSignerOptimized(new SHA256Digest(), w, g);
