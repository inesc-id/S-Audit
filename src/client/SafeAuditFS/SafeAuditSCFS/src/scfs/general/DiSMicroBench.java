...
public class DiSMicroBench {
    ...
    public static void main(String[] args) throws IOException {
        ...
        try {
            dis = new MetadataCacheOnSyncDirectoryService(new NoCacheDirectoryService(clientId,
                    init("microBench", false, clientId)));
        ...
        */
        ...
        NodeMetadata m = new NodeMetadata(NodeType.DIR, "/", "DIR", createDefaultFileStats(
                NodeType.DIR, start, 0), "" + clientId + start, defaultKey, new int[] { clientId },
                new int[] { clientId });
        try {
            dis.putMetadata(m);
        ...
        for (int i = 0; i < NUM_FILES_DIR; i++) {
            m = new NodeMetadata(NodeType.FILE, "/DIR", "000000000000000000000000000000" + i,
                    createDefaultFileStats(NodeType.FILE, start, 0), "" + clientId + start,
                    defaultKey, new int[] { clientId }, new int[] { clientId });
        }
        //PUT
        ...
        for (int i = 0; i < NUM_IT; i++) {
            ...
            start = System.currentTimeMillis();
            m = new NodeMetadata(NodeType.FILE, parent, "000000000000000000000000000000" + i,
                    createDefaultFileStats(NodeType.FILE, start, 0), "" + clientId + start,
                    defaultKey, new int[] { clientId }, new int[] { clientId });
            mList.add(m);
            try {
    ...
    private static double dp(int[] array, double mediaAritimetica) {
        if (array.length == 1) {
            ...
            return Math.sqrt(((double) 1 / (array.length - 1)) * somatorio);
        }
    }
    ...
    public static FileStats createDefaultFileStats(NodeType nodeType, long inode, int mode) {
        ...
        return new FileStats(inode, mode, nlink, uid, gid, rdev, size, blocks, atime, mtime, ctime,
                new byte[20], nodeType == NodeType.FILE ? true : false, true);
    }
