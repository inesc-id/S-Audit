...
/**
 * Class that represents all the request and replies for each cloud Note there is N object
 * of this class (each object e connected with each cloud)
 * 
 * @author tiago oliveira
 * @author bruno quaresma
 */
public class DepSkySCloudManager extends
        Thread {
    ...
    public DepSkySCloudManager(IDepSkySDriver driver,
                               ICloudDataManager cloudDataManager,
                               IDepSkySProtocol depskys) {
        this.driver = driver;
        ...
        try {
            ...
            switch (request.op) {
                case INIT_SESS:
                    ...
                    System.out.println("got response " + sessid);
                    //add the reply for this operation
                    addReply(new CloudReply(request.op, request.seqNumber, driver.getDriverId(),
                            sessid, request.cid, request.reg, request.protoOp,
                            request.isMetadataFile, request.hashMatching));
                    break;
                case DEL_CONT:

                    //Delete all the files in this container (metadata and data files)
                    boolean delContRes = driver.deleteContainer(request.reg.getBucketName(),
                            request.namesToDelete, request.accessToOtherAccount);

                    addReply(new CloudReply(request.op, request.seqNumber, driver.getDriverId(),
                            delContRes, request.cid, request.reg, request.protoOp,
                            request.isMetadataFile, request.hashMatching));
                    break;
                case NEW_DATA:
                    ...
                    String ssid = driver.uploadData(request.reg.getBucketName(), request.w_data,
                            request.did, request.accessToOtherAccount);

                    r = new CloudReply(request.op, request.seqNumber, driver.getDriverId(), ssid,
                            request.cid, request.reg, request.protoOp, request.isMetadataFile,
                ...
                case GET_DATA:
                    ...
                    byte[] data = driver.downloadData(request.reg.getBucketName(), request.did,
                            request.accessToOtherAccount);
                    //				if(driver.getDriverId().equals("cloud1"))
                    r = new CloudReply(request.op, request.seqNumber, driver.getDriverId(), data,
                            request.cid, request.reg, request.protoOp, request.isMetadataFile,
                            request.vNumber, request.vHash, request.allDataHash,
                            request.hashMatching, request.accessToOtherAccount,
                            request.numVersionToKeep);
                    r.setInitReceiveTime(init);
                    r.setStartTime(request.startTime);
                ...
                case DEL_DATA:
                    ...
                    boolean delRes = driver.deleteData(request.reg.getBucketName(), request.did,
                            null);

                    addReply(new CloudReply(request.op, request.seqNumber, driver.getDriverId(),
                            delRes, request.cid, request.reg, request.protoOp,
                            request.isMetadataFile, request.hashMatching));
                    break;
                case LIST:
                    ...
                    LinkedList<String> name = driver.listNames(request.did,
                            request.reg.getBucketName(), null);

                    r = new CloudReply(request.op, request.seqNumber, driver.getDriverId(), null,
                            request.cid, request.reg, request.protoOp, request.isMetadataFile,
                    ...
                    break;
                case SET_ACL:
                    String[] credentialsToShare = driver.setAcl(request.reg.getBucketName(),
                            request.canonicalId, request.permission);

                    r = new CloudReply(request.op, request.seqNumber, driver.getDriverId(),
                            credentialsToShare, request.cid, request.reg, request.protoOp,
                            request.isMetadataFile, request.hashMatching);
                    addReply(r);
                    break;
                default:
                    ...
                    addReply(new CloudReply(request.op, request.seqNumber, driver.getDriverId(),
                            null, request.cid, request.reg, request.protoOp,
                            request.isMetadataFile, null));
                    break;
            }
            ...
            r = new CloudReply(request.op, request.seqNumber, driver.getDriverId(), null,
                    request.cid, request.reg, request.protoOp, request.isMetadataFile,
                    request.vNumber, request.vHash, null, null, null, -1);
            r.setReceiveTime(System.currentTimeMillis());
            r.setInitReceiveTime(init);
        ...
        try {
            ...
            if (reply.response != null) {
                ...
                if (reply.type == SET_ACL) {
                    ...
                    CloudRequest r = new CloudRequest(GET_DATA, reply.sequence, ids[0], ids[1],
                            null, null, reply.reg, reply.protoOp, true, reply.hashMatching, null);
